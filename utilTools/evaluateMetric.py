...
if __name__ == '__main__':
    ...
    # step 1：读入图像，并灰度化
    GT_path = 'G:/scientific research/COVID19/covid19/GT/'
    DSI = []
    VOE = []
    RVD = []
    Precision = []
    ...
    for name in os.listdir(GT_path):
        imgGT_path = os.path.join(GT_path, name)
        imgR_path = os.path.join(GT_path.replace('GT', 'ans'), name)

        img_GT = cv2.imread(imgGT_path, 0)
        img_R = cv2.imread(imgR_path, 0)
        ...
        # step 4：计算DSI
        DSI.append(calDSI(binary_GT, binary_R))
        VOE.append(calVOE(binary_GT, binary_R))
        RVD.append(calRVD(binary_GT, binary_R))
        Precision.append(calPrecision(binary_GT, binary_R))
        Recall.append(calRecall(binary_GT, binary_R))

    print('（1）DICE计算结果，      DSI       = {0:.4}'.format(np.average(DSI)))  # 保留四位有效数字

    # step 5：计算VOE
    print('（2）VOE计算结果，       VOE       = {0:.4}'.format(np.average(VOE)))

    # step 6：计算RVD
    print('（3）RVD计算结果，       RVD       = {0:.4}'.format(np.average(RVD)))

    # step 7：计算Precision
    print('（4）Precision计算结果， Precision = {0:.4}'.format(np.average(Precision)))

    # step 8：计算Recall
    print('（5）Recall计算结果，    Recall    = {0:.4}'.format(np.average(Recall)))